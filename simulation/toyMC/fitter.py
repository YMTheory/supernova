    ...
    for idx, dT in enumerate(dt_arr):
        ...
        for cha in channels:
            if MO == "NO":
                ...
                if not reverse:
                    val += cha.calc_Asimov_NLL_NO(dT)
                    #print(dT, cha.calc_Asimov_NLL_NO(dT))
                else:
                    val += cha.calc_Asimov_NLL_IO(dT)
        nll[idx] = val
    return nll
...
def scanning_asimov1D_partlysep(dt_arr, channels, MO, tagIBD=1.0, tmin=-0.02, tmax=0.02, stept=0.00001):
    nll_combined = scanning_asimov1D_combined(dt_arr, channels, MO, scaleIBD=1-tagIBD)
    for cha in channels:
        if cha.name == "IBD":
            nll_partIBD = scanning_asimov1D_separate(dt_arr, [cha], MO) * tagIBD
    nll = nll_combined + nll_partIBD
    return nll
...
def scanning_chain(mode, dt_arr, channels, MO, tag_eff=0.5, tmin=-0.02, tmax=0.02, stept=0.00001, reverse=False):
    if mode == "1D_sep":
        nll_arr = scanning_asimov1D_separate(dt_arr, channels, MO, reverse=reverse)
    elif mode == "1D_comb":
        nll_arr = scanning_asimov1D_combined(dt_arr, channels, MO, tmin=tmin, tmax=tmax, stept=stept)
    elif mode == "1D_part":
        nll_arr = scanning_asimov1D_partlysep(dt_arr, channels, MO, tagIBD=tag_eff, tmin=tmin, tmax=tmax, stept=stept)
    Tbest, _, _ = find_locMin(dt_arr, nll_arr)
    ...
    if mode == "1D_sep":
        ...
        nll_arr = scanning_asimov1D_partlysep(dt_arr, channels, MO, tagIBD=tag_eff, tmin=tmin, tmax=tmax, stept=stept)
    Tbest, locMin, a, b, c = parabola_fit(dt_arr, nll_arr)

    return dt_arr, nll_arr, Tbest, locMin, a, b, c
