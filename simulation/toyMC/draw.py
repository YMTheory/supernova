...
def draw():
    ...
    _, _, sens_dataNO, _, _, sens_dataIO            = read("Garching82703", "pESeESIBD", 0.10, 20, 10, end="tot_doFit_binnedData_unbinnedNLL_C14bkghigh_PosiToyData")
    _, _, sens_dataNO_bin, _, _, sens_dataIO_bin    = read("Garching82703", "pESeESIBD", 0.15, 20, 10, end="tot_doFit_binnedData_unbinnedNLL_C14bkghigh_PosiToyData")

    fig, ax = plt.subplots(figsize=(9, 4))
    h1 = ax.hist(sens_dataNO,   bins=100, range=(-30, 30),   alpha=0.5, label="true NO (high C14)", color="blue",)
    h2 = ax.hist(-sens_dataIO,  bins=100, range=(-30, 30),  alpha=0.5, label="true IO (high C14)", color="red")
    h3 = ax.hist(sens_dataNO_bin,   bins=100, range=(-30, 30),   alpha=0.5, label="true NO (low C14)", histtype="step", lw=2, color="green",)
    h4 = ax.hist(-sens_dataIO_bin,  bins=100, range=(-30, 30),  alpha=0.5, label="true IO (low C14)", histtype="step", lw=2, color="orange")
    ...
    #h3 = ax.hist(sens_dataNO_bin,   bins=100, range=(-30, 30),   alpha=0.5, label="true NO (0.01 ms binning)", histtype="step", lw=2, color="green",)
    #h4 = ax.hist(-sens_dataIO_bin,  bins=100, range=(-30, 30),  alpha=0.5, label="true IO (0.01 ms binning)", histtype="step", lw=2, color="orange")
    ...
    ax.vlines(np.median(sens_dataNO_bin),  0, 2500, linestyle="-.", lw=2.5, color="green")
    ...
    plt.tight_layout()
    #plt.savefig("Garching82703_pESeESIBD_10kpc_binnedData_binnedNLL+unbinnedNLL.png")
    plt.show()
