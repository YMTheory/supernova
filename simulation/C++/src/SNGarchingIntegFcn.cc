...
SNGarchingIntegFcn::SNGarchingIntegFcn(){
...
SNGarchingIntegFcn::SNGarchingIntegFcn(int imode){
    for(int it=0; it<3; it++){
        grLuminosity[it] = NULL;
        grAlpha[it]      = NULL;
        grAverageE[it]   = NULL;
...
void SNGarchingIntegFcn::readFluxGraph(int imode){

    if (imode == 11 ) {
        std::cout << "YB mode is using here ..." << std::endl;
        ...
        if((!fSNmod_antinue) || (!fSNmod_nue) || (!fSNmod_nux)){
            std::cout << "Error: " << imode << " files of Garching models can't be opened!" << std::endl;
            exit(-1);
        }
    ...
    else if(imode<70000 || imode >99999){
        ...
        //read in data
        TString path="/junofs/users/miaoyu/supernova/wenlj/simulation/data/Garching";
        std::ifstream fSNmod_antinue;
        std::ifstream fSNmod_nue;
        std::ifstream fSNmod_nux;
        fSNmod_antinue.open(Form("%s/%d/timedata/neutrino_signal_nubar_e",path.Data(), imode));
        fSNmod_nue.open(Form("%s/%d/timedata/neutrino_signal_nu_e",path.Data(), imode));
        fSNmod_nux.open(Form("%s/%d/timedata/neutrino_signal_nu_x",path.Data(), imode));
...
double SNGarchingIntegFcn::fcnfluxtime(double* x, double* par){
    double time = x[0];
    double Ev   = par[0];
    int type = int(par[1]);
...
double SNGarchingIntegFcn::getNumT(double time, int type){
    double num=0;
    if(type < 2 ){
...
double SNGarchingIntegFcn::getLumT(double time, int type){
    double lumi=0;
    if(type < 2 ) {
        if(time < timeMin[type])return 0;
        if(time > timeMax[type])return 0;
...
double SNGarchingIntegFcn::getEventAtTime(double time, double E, int type){
    double luminosity, A, alpha;
    if(type < 2){
        if(time < timeMin[type]) return 0;
        if(time > timeMax[type]) return 0;
        ...
        if (A == 0 ) return 0 ;
        // test line
        //std::cout << "getEventAtTime: time, luminosity, A, alpha: (for neutrino )" << type  << " "
    ...
    //std::cout << "flavorID = " << type << " " << time << " " << E << " " << luminosity << " "
      //       << A << " " << alpha << " " << flux << std::endl;
    return index*flux;
}

double SNGarchingIntegFcn::getAverageET(double time, int type){
    double aveE = 0;
