...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(tarr))):
        t = tarr[i]
        arr[i, 0] = t
        arr[i, 1] = spec.getVisibleEventAtTEvisIntegral_root(t, Evismin, Evismax, 0)
    ...
    np.savetxt(f"/junofs/users/miaoyu/supernova/simulation/data/{cha}_Emin{Emin:.2f}MeVEmax{Emax:.2f}MeV_tmin{tmin:.3f}stmax{tmax:.3f}s.txt", arr)
