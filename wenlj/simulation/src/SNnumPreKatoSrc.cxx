...
SNnumPreKatoSrc::SNnumPreKatoSrc() : SNsource(){
    ppreInteg = NULL;
    f1TE = NULL;
    setSNmaxEv(25);
}

SNnumPreKatoSrc::SNnumPreKatoSrc(int imode) : SNsource(){
    ppreInteg = new SNpreKatoIntegFcn(imode);
    f1TE = new TF1("f1TE",ppreInteg, &SNpreKatoIntegFcn::fluxTimeE,0,40,3,"SNpreKatoIntegFcn","fluxTimeE");
}

SNnumPreKatoSrc::~SNnumPreKatoSrc(){
...
double SNnumPreKatoSrc::oneSNFluenceDet(double E, int type){
    double index = 1./(4*TMath::Pi()*TMath::Power(dist*3.086e21,2));//cm^-2
    double flux = ppreInteg->getFluxE(E, type);
    return flux*index;
}

double SNnumPreKatoSrc::oneSNFluenceDet(double E, int type, int MH){
    double fluence = 0;
...
double SNnumPreKatoSrc::totalSNFluenceDet(double E){
    int ntype = 6;
    double totflu = 0;
...
double SNnumPreKatoSrc::totalSNFluenceDet(double E, int MH){
    int ntype = 6;
    double tfluence=0;
...
double SNnumPreKatoSrc::oneSNFluenceDetTimeIntegE(double time, int type, int MH){
    double index = 1./(4*TMath::Pi()*TMath::Power(dist*3.086e21,2));//cm^-2
    if(MH == 0){
...
double SNnumPreKatoSrc::totalSNFluenceDetTimeIntegE(double time, int MH){
    int ntype = 6;
    double totflu = 0;
    for(int it=0; it<ntype; it++){
...
double SNnumPreKatoSrc::oneSNFluenceDetAtTime(double time, double E, int type, int MH){
    double index = 1./(4*TMath::Pi()*TMath::Power(dist*3.086e21,2));//cm^-2
    if(MH == 0){
...
double SNnumPreKatoSrc::totalSNFluenceDetAtTime(double time, double E, int MH){
    int ntype = 6;
    double totflu = 0;
    for(int it=0; it<ntype; it++){
...
double SNnumPreKatoSrc::oneSNFluenceDetTimeInterval(double E, double tfirst, double tlast, int type){
    double index = 1./(4*TMath::Pi()*TMath::Power(dist*3.086e21,2));//cm^-2
    f1TE->SetParameters(tfirst,tlast,type);
    double flux = f1TE->Eval(E);
    return flux*index;
}
double SNnumPreKatoSrc::totalSNFluenceDetTimeInterval(double E, double tfiirst, double tlast){
...
double SNnumPreKatoSrc::oneSNFluenceDetTimeInterval(double E, double tfirst, double tlast, int type, int MH){
    double fluence = 0;
    double sin2theta12 = 0.297;
    double sin2theta13 = 0.022;
    double oscp, oscpbar;
...
double SNnumPreKatoSrc::totalSNFluenceDetTimeInterval(double E, double tfirst, double tlast, int MH){
...
double SNnumPreKatoSrc::oneSNLuminosityTime(double time, int type){
    double lumi = ppreInteg->getLumT(time, type);
    return lumi;
}

double SNnumPreKatoSrc::oneSNAverageETime(double time, int type){
    return ppreInteg->getAverageET(time, type);
}


void SNnumPreKatoSrc::getTimeRange(double& tmin, double& tmax, int type){
