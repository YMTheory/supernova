...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 6:
        print("Usage: %s [modelNum] [channel] [nuType] [nuMass] [dist]"  % (sys.argv[0]))
            ...
            if chaname == snDet.NuE:
                for j in range(nbins_Ev):
                    ...
                    if fluence > 0. and dxs >0.:
                        ...
                        evisFactor = step_Ev/step_Evis
                        h2d_etSpec[MH].Fill(timeTmp[0], EvisTmp, Npar*fluence*dxs*evisFactor*tFactor)

                        evisFactor = step_Evis/step_Ev
                        h2d_eNuTSpec[MH].Fill(timeTmp[0], EvTmp, Npar*fluence*dxs*evisFactor*tFactor)
