...
if __name__ == "__main__":
    ...
    #for iSub in range(4,5):
        ...
        for iScan in range(NScans):
            dT = dTexp + (2*iScan-NScans)*dTstep/2 + 2.5e-5
            ...
            #absNLL = fitPdf1D.createNLL(fitdata, ROOT.RooFit.Range('nllRange'))
            #nllVal = absNLL.getVal()
            #print('nsig.getVal(): ', nsig.getVal())
        ...
        for iScan in range(NScans):   
            ...
            #print('dT, absNLL, nllVal: %10.6f, %10.3f, %10.3f'%(dT, absNLL.getVal(), nllVal))
            #newTList.sort()
            #tMin = newTList[0]
            #tMax = newTList[len(newTList)-1]
            #tMinBin = inputHist1D.FindBin(tMin)
            #tMaxBin = inputHist1D.FindBin(tMax)
            #if dT>-0.0005 and dT<-0.0004:
            #    print('test, min, Tmin+dT: ', tMin, inputHist1D.GetBinLowEdge(tMinBin), Tmin+dT)
            #    print('test, max, Tmax+dT: ', tMax, inputHist1D.GetBinLowEdge(tMaxBin), Tmax+dT)
                
            #if iScan == 10:
            #    for tTmp in newTList:
            #        print('{0:10.6f}, '.format(tTmp), end='')
            #    print('end')

            #xframe = nuTime.frame(ROOT.RooFit.Title("data group %d, dT: %8.5f"%(iSub, dT)), 
            #                ROOT.RooFit.Range('nllRange'), ROOT.RooFit.Bins(100))
            #xframe.GetYaxis().SetTitleOffset(1.4)
            ##xframe.SetTitle("data group %d, dT: %8.5f"%(iSub, dT))
            #can.cd()
